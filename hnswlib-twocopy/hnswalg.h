...
//                    if (candidate_id == 0) continue;
    ...
    */
    ...
    searchKnnAMX(const void *query_data, size_t k, BaseFilterFunctor* isIdAllowed = nullptr) const {
        ...
        size_t dim=(size_t)(*(size_t *)dist_func_param_);
        if(mydata==NULL){
          mydata=(void*)malloc(sizeof(dist_t)*maxM0_*dim);
...
/*                 void *mydata=(void*)malloc(sizeof(dist_t)*size*dim);
                float *res=(float*) malloc(size*sizeof(float));
                memset(res,0,size*sizeof(float)); */

                //printf("size:%d sizeof(dist_t):%d dim:%d\n",size,sizeof(dist_t),dim);
                for (int i= 0; i<size;i++){
                  tableint cand = datal[i];
...
/*                   int32_t scalar_result[32]={0};
                  for(int i=0;i<size;i++){
                      scalar_result[i]+=vector_dot_product_int32_t((void*)query_data,(void*)mydata+i*dim*sizeof(dist_t),dist_func_param_);
                  }  */
                   memset(res,0,sizeof(int32_t)*size);
                  // devided_batch_amx_inner_product((int8_t**)&query_data,(int8_t**)&mydata,dim,1,size,res);
